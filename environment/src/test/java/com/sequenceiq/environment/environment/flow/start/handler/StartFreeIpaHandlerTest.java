...
class StartFreeIpaHandlerTest {
    ...
    public void shouldThrowErrorWhenUnstartable(Status status) {
        ...
        verifyEnvStartFailedEvent(environmentDtoEvent, status);
    }
    ...
    private void verifyEnvStartFailedEvent(Event<EnvironmentStartDto> environmentDtoEvent, Status status) {
        ...
        Assertions.assertThat(envStartFailedEventCaptor.getValue())
                ...
                .returns("FreeIPA is not in a valid state to start! Current state is: " + status, event -> event.getException().getMessage());
    }
