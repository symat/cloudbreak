...
public class InternalSdxSshAndCmAccessTest extends PreconditionSdxE2ETest {
    ...
    @Description(
            ...
            then = "SSH and Cloudera Manager access should be successful with new workload password, 'uuid' file and expected mappings should be present"
    )
    public void testSdxSshAndCmAccessWithNewWorkloadPassword(TestContext testContext) {
        ...
        String sdx = testContext.given(SdxInternalTestDto.class).when(sdxTestClient.describeInternal()).getResponse().getName();
        ...
        testContext
                ...
                .when(freeIpaTestClient.getLastSyncOperationStatus(), key(freeIpa))
                ...
                .given(FreeIpaUserSyncTestDto.class)
                .when(freeIpaTestClient.syncAll(), key(freeIpa))
                .await(OperationState.COMPLETED)
                ...
                .then((tc, testDto, client) -> clouderaManagerUtil.checkClouderaManagerKnoxIDBrokerRoleConfigGroups(tc, testDto, client, workloadUsername,
                        newWorkloadPassword))
                .validate();
    }
}
